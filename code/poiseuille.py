...
# TODOs:
# 1. Convert to class form
# 2. Sparse matrix and use sparse eigen solver: https://docs.scipy.org/doc/scipy/reference/generated/scipy.sparse.linalg.eigs.html
# 3. Sparse linear solver (for the adjoint equation).
...
class poiseuille(object):
    ...
    def setup(self, alp, beta, R):

        self.set_Dmat()
        self.set_pois(alp, beta, R)

    def solve(self, isFull = True):

        A = self.A
        B = self.B

        if isFull:
            # If solve the full spectrum
            w, vr = sp.linalg.eig(A, b = B)
        else:
            # If only the dominant one is of interest
            BinvA = np.linalg.inv(B).dot(A)
            w, vr = sp.sparse.linalg.eigs(BinvA, k = 1, which="LI")
    ...
    def get_JT(self, A, lambda0, phi0):
...
#     plt.savefig('../doc/figures/poiseuille_eig.pdf',bbox_inches='tight')
#     plt.show()
